...
static void _sms4_set_encrypt_key(u32 key[SMS4_NUM_ROUNDS], const u8 *user_key)
{
    u32 *rk = key;
    u32 X0, X1, X2, X3, X4;
...
static void _sms4_set_decrypt_key(u32 key[SMS4_NUM_ROUNDS], const u8 *user_key)
{
    u32 *rk = key;
    u32 X0, X1, X2, X3, X4;
...
static void _sms4_encrypt(const u8 *in, u8 *out, const u32 key[SMS4_NUM_ROUNDS])
{
    const u32 *rk = key;
    u32 X0, X1, X2, X3, X4;
...
{
    struct crypto_sms4_ctx *ctx = crypto_tfm_ctx(tfm);

    if(keylen != SMS4_KEY_LENGTH)
    {
        tfm->crt_flags |= CRYPTO_TFM_RES_BAD_KEY_LEN;
        return -EINVAL;
    }

    _sms4_set_encrypt_key(ctx->key_enc, key);
    _sms4_set_decrypt_key(ctx->key_dec, key);
    return 0;
}
EXPORT_SYMBOL_GPL(crypto_sms4_set_key);

static void sms4_encrypt(struct crypto_tfm *tfm, u8 *out, const u8 *in)
{
    struct crypto_sms4_ctx *ctx = crypto_tfm_ctx(tfm);
    _sms4_encrypt(in, out, ctx->key_enc);
}

static void sms4_decrypt(struct crypto_tfm *tfm, u8 *out, const u8 *in)
{
    struct crypto_sms4_ctx *ctx = crypto_tfm_ctx(tfm);
    _sms4_encrypt(in, out, ctx->key_dec);
}

static struct crypto_alg sms4_alg = {
    .cra_name           =   "sms4",
    .cra_driver_name    =   "sms4-generic",
    .cra_flags          =   CRYPTO_ALG_TYPE_CIPHER,
    .cra_blocksize      =   SMS4_BLOCK_SIZE,
    .cra_ctxsize        =   sizeof(struct crypto_sms4_ctx),
    .cra_alignmask      =   3,
    .cra_priority       =   100,
    .cra_module         =   THIS_MODULE,
    .cra_list           =   LIST_HEAD_INIT(sms4_alg.cra_list),
    .cra_u              =   {
        .cipher = {
            .cia_min_keysize    =   SMS4_KEY_LENGTH,
            .cia_max_keysize    =   SMS4_KEY_LENGTH,
            .cia_setkey         =   crypto_sms4_set_key,
            .cia_encrypt        =   sms4_encrypt,
            .cia_decrypt        =   sms4_decrypt
        }
    }
};

static int __init sms4_init(void)
{
    return crypto_register_alg(&sms4_alg);
}

static void __exit sms4_fini(void)
{
    crypto_unregister_alg(&sms4_alg);
}

module_init(sms4_init);
module_exit(sms4_fini);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("lijk <lijk@infosec.com.cn>");
MODULE_DESCRIPTION("SMS4 Cipher Algorithms");
